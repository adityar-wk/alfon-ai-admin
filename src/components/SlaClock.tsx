...
export function SlaClock({ sla, className = "" }: { sla: Task["sla"]; className?: string }) {
  ...
  const tone = secs <= 120 ? "text-red-600" : sla.kind === "due" ? "text-orange-600" : "text-ink-secondary";
  return (
    <span className={`inline-flex items-center gap-1.5 whitespace-nowrap font-display text-[14px] font-semibold tabular-nums ${tone} ${className}`}>
