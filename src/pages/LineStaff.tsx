...
const INFO: Record<Tab, { title: string; blurb: string; flows: string[]; included: string[]; excluded: string[] }> = {
  line: {
    ...
    flows: [
      ...
      "Bell → Notifications",
      "Orange + button (bottom right) → Create Manual Task (department, service, severity)",
      "Availability toggle: Available / Off work",
    ],
    included: ["SLA timer on every task", "Room, guest and preference details", "Escalate to supervisor when blocked", "Manual task with severity"],
  ...
  manager: {
    ...
    flows: [
      ...
      "Escalation review: send back with instruction, take ownership, route to another department, escalate to GM",
      "+ button → Create Manual Task for the department (service, severity, room, details)",
